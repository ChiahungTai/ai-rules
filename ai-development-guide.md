...
> **🔴 強烈警告：`sed` 替換操作的高風險**
>
> `sed` 是流編輯器，**完全不理解**程式碼或 Markdown 的語法結構。使用 `sed` 進行批次替換常造成以下災難性後果：
>
> - **破壞縮排**：Python 程式碼依賴縮排，`sed` 可能破壞語意
> - **誤替換註解/字串**：`sed/g` 會替換「匹配模式」的所有出現，包括註解、字串內容
> - **破壞 Markdown 結構**：`sed` 不理解標題、程式碼區塊、表格語法
> - **破壞多行結構**：`sed` 逐行處理，不理解跨行語法（括號配對、多行字串）
> - **不可預測的結果**：`sed -i` 直接修改檔案，錯誤無法還原（除非有備份）
>
> **❌ 絕對禁止使用 `sed` 的場景**：
