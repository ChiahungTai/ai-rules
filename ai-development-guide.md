...
4. **原子操作**：Capabilities 新增 + Kanban card 移動必須同時完成（`/doc-health` angle 6 驗證）
5. **EP 歸檔**：所有段落 commit 完成 → 提醒用戶歸檔 EP 至 `_done/`
...
/spec（建立 Kanban Backlog card）→ /execution-plan（引用 UC ID + SYSTEM-MAP 關聯）→ [/ep-validate] → /build（階段1: Backlog→InProgress；提煉消費場景，不動 Capabilities/Done）
    → /code-review（驗證 Capabilities 覆蓋度）→ /commit（確認後更新 Capabilities + 搬 Kanban Done + EP 歸檔提醒）→ /task-status
```
...
| `/commit` | 確認後執行：CLAUDE.md Capabilities 表格新增 ✅ 行 + Kanban card 移至 Done/（原子操作）；EP 所有段落完成時提醒歸檔 |
