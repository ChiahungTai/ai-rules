...
2. **/commit → Capabilities + Done**：用戶確認 commit 後，在 CLAUDE.md Capabilities 表格新增一行，同時移動 Kanban card 至 Done/（不在 /build 執行，避免 build 後不 commit 造成狀態不一致）
3. **原子操作**：Capabilities 新增 + Kanban card 移動必須同時完成（`/doc-health` angle 6 驗證）
4. **EP 歸檔**：所有段落 commit 完成 → 提醒用戶歸檔 EP 至 `_done/`
...
```
/spec（建立 Kanban Backlog card）→ /execution-plan（引用 UC ID + SYSTEM-MAP 關聯）→ [/ep-validate] → /build（提煉消費場景，不動 Capabilities/Kanban）
...
| `/build` | 段落完成後：從 EP Scenario Matrix 提煉消費場景（暫存，供 `/commit` 使用）；更新 SYSTEM-MAP.md 功能生命週期狀態；**不更新 Capabilities、不搬 Kanban 卡片**（避免 build 後不 commit 造成狀態不一致） |
