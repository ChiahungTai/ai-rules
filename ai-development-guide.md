...
> **核心原則**: 架構設計必須從基本原理出發，不依賴類比推測或常見做法
...
**驗證**: src/core/pipeline.py 處理了這些需求"
```
...
**問題**：
- 「pydantic 應該是這樣運作的…」→ 未實際查證
- 「我記得這個函數的參數是…」→ 記憶可能過期或錯誤
- 「大部分專案都這樣用…」→ 未驗證適用性

## ✅ 正確做法：查證 .venv 中的套件源碼

```markdown
AI: "關於 pydantic BaseModel 的驗證行為，讓我查證實際源碼：

**步驟 1**: 確認套件位置
`uv pip list | grep pydantic` → pydantic 2.5.0

**步驟 2**: 讀取核心源碼
`Read .venv/lib/python3.12/site-packages/pydantic/main.py`

**步驟 3**: 驗證發現
- BaseModel.__init__ 在 main.py:177-189
- 驗證邏輯在 _validate_call 函數中
- 與我記憶不同，實際上先呼叫 __pydantic_validator__ 而非 field_validator
...
### **實例分析：AI 使用 sed 替換程式碼**

> **本節展示如何用第一性原理分析技術選型問題
...
| 狀況 | 條件 | 範例 |
|------|------|------|
| **配置文件** | 簡單 key=value 格式 | `.env` 檔案中的字串替換 |
| **純文字檔案** | 無程式碼結構 | `README.md` 中的版本號更新 |
| **日誌處理** | 格式化的輸出 | 過濾或格式化日誌內容 |
| **批次重命名** | 檔名規則替換 | 批次修改檔名 |

**判斷準則**：使用 sed 前，確認是純文字、替換目標唯一、不涉及多行結構、不需理解語意。
