...
```
/spec（建立 Kanban Backlog card）→ /execution-plan（引用 UC ID + SYSTEM-MAP 關聯）→ [/ep-validate] → /build（階段1: Backlog→InProgress；提煉消費場景，不動 Capabilities/Done）
...
| `/build` | 階段 1：掃描 EP UC ID，搬 Kanban Backlog→InProgress（暫時狀態）；段落完成後提煉消費場景（暫存，供 `/commit` 使用）；更新 SYSTEM-MAP.md 功能生命週期狀態；**不更新 Capabilities、不搬 Done**（永久狀態在 `/commit` 確認後才更新） |
