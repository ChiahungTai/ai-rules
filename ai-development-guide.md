...
| 情境 | 觸發 | UC 路徑 | GAPS |
|------|------|---------|------|
| **A: 一次到位** | /spec → EP → /build 完成 | 📋→✅ | 不需要 |
| **B: 分段完成** | /spec → EP → /build 部分 | 📋→部分✅ | 剩餘段寫入或更新 GAP |
| **C: 回溯盤點** | 已有程式碼 | ✅ 或 📋 | 📋 項寫入或更新 GAP |
| **D: 棄用清理** | 功能淘汰 | ❌ | 清理項寫入或更新 GAP |

**GAPS 產生規則**：只在「無法一次完成」時產生。一次到位不需 GAP。GAPS.md 中每個 GAP 必須引用對應 UC ID。
...
| `/build` | 段落完成後更新 UC 狀態（📋→✅）；未完成段落寫入或更新 GAPS.md |
| `/code-review` | 第六軸：UC 覆蓋度（實作是否滿足 UC 描述） |
| `/commit` | 大型/中型變更時提示確認 UC 狀態更新 |
| `/standup` | UC 進度摘要（狀態變化、新增、關閉） |
| `/uc-status` | 全局進度掃描（USE-CASES + GAPS 跨領域儀表板） |

### 文件放置

- `USE-CASES.md`：按領域/模組目錄放置，每個 UC 有唯一 ID（如 D-01、B-06）
- `GAPS.md`：與 USE-CASES.md 同目錄，每個 GAP 引用對應 UC ID
- 全局進度：`/uc-status` 掃描所有 USE-CASES.md + GAPS.md
