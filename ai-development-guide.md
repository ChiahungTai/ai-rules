...
| 原則 | 說明 | 應用場景 |
|------|------|----------|
| **Complete Execution** | 完整執行，絕不半成品 | 方向明確時的任務執行 |
| **Breaking Change First** | 破壞性變更優先，需求確認 | 架構重構和API設計 |
...
```markdown
# ❌ 錯誤：急著交差了事
AI: "功能基本完成了，還有幾個小測試要修，不過核心功能可以用"
...
> **核心原則**: 預設不考慮向後相容，特殊需求必須明確確認

#### **預設立場**
- ✅ **預設破壞性變更**: 以品質和功能完整度為優先，不考慮向後相容
- ✅ **架構優先**: 可以進行大幅重構來改善代碼架構
- ✅ **現代化導向**: 採用最佳實踐和新技術，不受舊有兼容性束縛

#### **確認機制**
只有當遇到以下情況時，才必須與使用者確認向後相容需求：
- 🔄 **API 介面變更**: 影響外部系統整合
- 🔄 **數據格式變更**: 影響現有數據處理流程
- 🔄 **配置文件變更**: 影響現有部署環境
- 🔄 **用戶明確要求**: 使用者主動提出向後相容需求

#### **範例對比**

```markdown
# ❌ 錯誤：假設需要向後相容
AI: "為了保持相容性，我保留了舊的API介面..."

# ✅ 正確：預設不考慮向後相容
AI: "我重構了API介面以提升架構品質。
如果您需要向後相容，請告訴我具體的相容需求，我會提供相應的解決方案。"
```
...
- **品質優先**: 可以大幅重構來改善架構品質
- **快速迭代**: 優先正確性和清晰度
- **向後相容**: **預設不考慮向後相容**，如有特殊需求必須與使用者明確確認
