...
三層文件各司其職：
- **CLAUDE.md**（架構 + 能力）：Capabilities 表格記錄已完成能力
- **SYSTEM-MAP.md**（功能狀態）：跨域功能狀態總覽
- **.kanban/**（任務管理）：📋/🔧 條目以 Kanban cards 存在

**UC 生命週期**：
- 📋 階段：/spec 建立 Kanban Backlog card（UC 描述在此定義）
- 🟡 階段：/build 階段 1 將 card 移至 In-Progress/（進行中）
- ✅ 階段：/commit 確認後 → CLAUDE.md Capabilities table 新增一行 + card 移至 Done/

**跨文件引用**：能力描述和入口路徑是索引鍵，貫穿 CLAUDE.md（✅）、Kanban cards（📋/🔧）、SYSTEM-MAP.md（功能引用）。
...
| 能力 | 入口 | 狀態 |
|------|------|------|
| 每日增量 K 線更新 | CLI `daily-update` / `run_daily_update()` | ✅ |

**Kanban card 格式**（📋/🔧 條目）：每張卡片有 frontmatter（模組、標題、spec 連結）+ body（設計要點、測試計畫、剩餘細節）。

### 四種開發情境

| 情境 | 觸發 | UC 路徑 |
|------|------|---------|
| **A: 一次到位** | /spec → EP → /build 完成 | 📋(Backlog) → 🟡(In-Progress, /build) → ✅(Capabilities, /commit) |
| **B: 分段完成** | /spec → EP → /build 部分 | 📋(Backlog) → 🟡(In-Progress, /build) → 🔧(Kanban，內嵌剩餘細節) |
...
> **核心原則**：✅ 條目歸屬於**主要實作所在的 library 模組目錄**的 CLAUDE.md，📋/🔧 條目以 Kanban cards 管理。
...
**跨域資訊歸屬**：
- **跨域依賴**：Capabilities 表格的「領域依賴」欄位引用其他能力描述
- **跨域流程**：SYSTEM-MAP.md 按工作流組織功能區塊，引用能力名稱
...
兩者是同一 UC 的兩個正交視角。✅ 時寫入 CLAUDE.md（永久），📋/🔧 時存在 .kanban/（暫時）。
...
1. **/spec → Kanban Backlog**：新建 UC 時建立 .kanban/Backlog/ card（含模組、spec 連結）
2. **/build → Kanban InProgress**：階段 1 將 Backlog cards 搬至 In-Progress/（暫時狀態，反映「正在做」）
3. **/commit → Capabilities + Done**：用戶確認 commit 後，在 CLAUDE.md Capabilities 表格新增一行，同時移動 Kanban card 至 Done/（永久狀態更新不在 /build 執行，避免 build 後不 commit 造成狀態不一致）
4. **原子操作**：Capabilities 新增 + Kanban card 移動 + EP 歸檔必須同時完成（`/doc-health` angle 6 驗證）
5. **EP 歸檔**：所有段落 commit 完成 → 自動歸檔 EP 至 `_done/`
...
|---|------|------|---------|------------|---------|
| SM-1 | 每日例行 | launchd / 手動 | Full pipeline | 無（從頭算） | 每日收盤完整流程 |
| SM-2 | 改 ranking 重跑歷史 | 手動 --date | 讀 snapshot → Analysis → JSON | snapshot | 每日收盤完整流程（部分） |
...
- **對應能力**：本場景引用的能力描述（可能對應多個能力，或僅部分步驟）
...
```
/spec（建立 Kanban Backlog card）→ /execution-plan（引用能力描述 + SYSTEM-MAP 關聯）→ [/ep-validate] → /build（階段1: Backlog→InProgress；提煉消費場景，不動 Capabilities/Done）
    → /code-review（驗證 Capabilities 覆蓋度）→ /commit（確認後更新 Capabilities + 搬 Kanban Done + EP 歸檔）→ /task-status
```

| Command | UC 職責 |
|---------|--------|
| `/spec` | 啟動時掃描相關 **CLAUDE.md Capabilities** + SYSTEM-MAP.md；建立 Kanban Backlog card |
| `/execution-plan` | EP 段落引用能力描述（如「實作每日收盤 Pipeline」）；掃描 SYSTEM-MAP.md 取得功能上下文；大型/中型變更須產出 Scenario Matrix |
| `/build` | 階段 1：掃描 EP 能力描述，搬 Kanban Backlog→InProgress（暫時狀態）；段落完成後提煉消費場景（暫存，供 `/commit` 使用）；更新 SYSTEM-MAP.md 功能生命週期狀態；**不更新 Capabilities、不搬 Done**（永久狀態在 `/commit` 確認後才更新） |
| `/code-review` | 第六軸：Capabilities 覆蓋度（實作是否滿足 **CLAUDE.md Capabilities** 描述） |
| `/commit` | 確認後執行：CLAUDE.md Capabilities 表格新增 ✅ 行 + Kanban card 移至 Done/ + EP 歸檔至 `_done/`（原子操作） |
...
- **CLAUDE.md Capabilities**：放在**主要實作所在的 library 模組目錄**（如 `mosaic_alpha/data/CLAUDE.md`），每個 ✅ UC 有精確的能力描述和入口路徑
...
| 狀態 | 含義 | 升級條件 | 降級條件 | 能力完成度 |
|------|------|---------|---------|-----------|
| 🏃 Running | 透過排程或工作流每日自動運行 | — | 自動化流程中斷或停止 → ⚠️ | 功能內能力多為 ✅ |
| ✅🔍 Verified | 程式碼完整 + 測試通過 + 整合環境驗證過 | 所有能力 ✅ + 測試通過 + 已在整合流程消費 | — | 功能內所有能力 ✅ |
| ✅ Built | 程式碼存在 + 測試通過，未在整合環境驗證 | 完成整合驗證 → ✅🔍 | 發現問題 → ⚠️ | 功能內所有能力 ✅ |
| ⚠️ Issues | 運行中或已完成但有已知問題 | 問題修復 → ✅🔍 或 ✅ Built | — | 部分能力可能有 📋/🔧 |
| 📋 Planned | 尚未實作 | 開始實作 → 🟡 或直接 ✅ Built | — | 功能內能力多為 📋 |
| ❌ Abandoned | 已廢棄，不再規劃 | — | — | 能力標記 ❌ |

**判定流程**：由 Capabilities + Kanban 狀態聚合推導。功能內所有能力 ✅ + 有整合驗證 → ✅🔍；全部 ✅ 但無整合驗證 → ✅ Built；有 📋/🔧 → 視問題嚴重度 ⚠️ 或 📋。`/doc-health --sync-system-map` 自動執行此推導。
