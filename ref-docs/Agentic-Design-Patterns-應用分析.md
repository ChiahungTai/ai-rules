...
書(Ch1 末 + Appendix A)把 **Context Engineering 提升為獨立紀律,位於 prompt engineering 之上**:「output 品質取決於 context 豐富度,非模型架構」;context 分層 = system prompt + 外部資料(retrieved docs + tool outputs)+ implicit data(user identity/history/env)。**這正是 ai-rules 的靈魂載體** —— `context-engineering` skill + `instruction-writing.md` Encoder Philosophy(CLAUDE.md = 概念壓縮表示)+ 載體選擇(context 何時載入)。ai-rules 本質上就是一個 context-engineering 系統,書把這個隱含定位說清楚了。(Appendix A「Pydantic parse-don't-validate at boundaries」= api-and-interface-design Validate-at-Boundaries 的 cousin。)
