...
| **CLAUDE.md 治理工具（harness-neutral 化）** | `rules/claude-writing.md`、`commands/claude/{init,clean,sync,distill}.md`、`skills/CLAUDE.md`、`commands/CLAUDE.md` |
...
| CLAUDE.md 相關 skills/commands AGENTS.md-aware（UC-3） | 📋 | `claude-writing.md`、`/claude:*`、`skills|commands/CLAUDE.md` |
...
| SM-5 | neutral 純度 | 寫入 Claude 專用 body 散文進 AGENTS.md | `claude-writing.md` 約定 + `/claude:sync` 擋下 | UC-3 |
...
- **可複用基礎設施**：`@` transclusion 機制權威在 `rules/claude-writing.md`（17 處 CLAUDE.md 引用）+ `skills/CLAUDE.md`——這兩檔是「如何寫 instruction file」的單一真相源，harness-neutral 化的核心。
...
**逐檔**：
1. **`rules/claude-writing.md`**（17 引用，@ 機制權威）——核心：
...
2. **`commands/claude/init.md`**（25 引用，產生 CLAUDE.md）：產出邏輯改為「產 `AGENTS.md`（neutral）+ 選擇性 thin `CLAUDE.md`（@wrapper）」
3. **`commands/claude/clean.md`**（32 引用）：清理對象加 `AGENTS.md`（neutral 純度檢查：擋 Claude body 散文）
4. **`commands/claude/sync.md`**（10 引用）：doc↔code sync 對象含 `AGENTS.md`
5. **`commands/claude/distill.md`**：蒸餾對象 `AGENTS.md`
...
- `claude-writing.md` 含「AGENTS.md 為 source」「@ 是 Claude 專用」「neutral 純度」三要素
- `/claude:init`、`/claude:clean`、`/claude:sync` description 對象含 AGENTS.md
...
- **回歸**：每段完成跑 `/claude:sync`（現版）確認文檔一致性；S4 是整體回歸。
...
1. **受影響命令/rules 行為反映**：`commands/CLAUDE.md` 命令索引 description 同步（`/claude:init`、`/claude:clean`、`/claude:sync`、`/claude:distill` 的 description 反映 AGENTS.md-aware）；`rules/claude-writing.md` 新約定生效。
