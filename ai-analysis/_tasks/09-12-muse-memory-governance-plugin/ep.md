...
| 最小 plugin skeleton | probe `p2/`（manifest＋deny 腳本＋fixture） | S1 manifest 起手模板 |
...
| hook 往返 smoke | `hooks/verify-memory-topology.sh:60-67` | S4 cutover 後改造指向 plugin 腳本 |

### 依賴關係與關鍵約束

- 控制面消費端（改動須同步）：`AGENTS.md:108`（專案結構 Muse memory 段）、`skills/memory-audit/SKILL.md:78`（hook 承載形態句）、`hooks/AGENTS.md:37`＋`hooks/MULTI-MACHINE.md:23,28`（setup-muse-hooks 條目）、`hooks/verify-memory-topology.sh:60-67`。rg 驗證：`rg -l "muse_memory_inbox|setup-muse-hooks" --glob '!backlog' --glob '!.agent-tmp'`＝上述集合（09-12 掃描）。
...
| SM-3 | marker 宣告了但壞掉 | JSON parse 失敗／`protocol` 缺 | **deny＋報錯、不落地**（禁 fail-open） | 修 marker 後重試 | 新增 UC |
| SM-4 | marker 是未來版本 | `protocol: 2`（> 支援版） | deny＋報錯（unsupported） | 升級 plugin | 新增 UC |
| SM-5 | 遷移疊窗：legacy 閘還在 | `.muse/hooks.json` memory matcher 在場 | plugin **no-op**（exit 0，legacy 全權）——零 dual-writer | 逐 repo cutover 完刪 shim | 新增 UC |
| SM-6 | bridge headless 委派 muse 寫 memory | bridge `task --family muse` 於 ai-rules | hook 照 fire＋inbox receipt（UNKNOWN 待證） | S2 gate ③ | 新增 UC |
...
| SM-11 | 非 git 目錄跑 muse | 無 repo 可解析 | allow native（等同 SM-2） | 無 | 新增 UC |
| SM-12 | hostile repo 放 marker＋symlink inbox | inbox 逐段 lstat 命中 symlink | deny＋報錯、**不落地**（導流寫入拒絕） | 無 | 新增 UC |
...
- **語義約束**：與 S4 共享「marker 檔名＝`.agents/memory-governance.json`、`protocol:1`」；與 S2 共享「stdin workspace 欄位探測優先序」；與全部段落共享「inbox payload 格式＝legacy 位元組級相容」。
...
- **受影響 domain invariant**：「muse 寫入必經 inbox」（memory pool 單一寫入點拓撲，AIR-54 契約）——閘的 fail-open/fail-closed 語義直接決定此 invariant 存亡。
- **critical path 觸及**：silent-corruption path——閘靜默失效（fail-open）＝muse 直寫池、治理繞過且無 crash 訊號。
- **驗證對齊**：三態矩陣測試（marker absent/malformed/unsupported/valid）＋governed 內部故障（jq 缺失模擬）→ deny 測試；第二道防線不變（consolidation 波前 `git -C <池> status` 異常篩＋daily-maintain Phase 0 直寫偵測——`skills/memory-audit/SKILL.md:82`、`skills/daily-maintain/SKILL.md:45`）。
...
2. **腳本決策序**（每步早退，最快路徑在前）：
   ...
   ② repo 解析（決策⑦）：stdin host workspace 欄位優先（欄位名＝S2 P-WS 凍結，腳本以候選序探測＋容忍缺席）→ 缺席走 `git rev-parse --show-toplevel`（$PWD 向上）；雙失敗 → exit 0（SM-11）。**禁假設 $PWD==workspace root**。
   ③ marker 三態（決策④）：`<repo>/.agents/memory-governance.json`——absent → exit 0（SM-2）；jq parse＋`protocol` 為整數 1 → 進 ④；malformed／`protocol` 缺／>1 → **deny＋reason（不落地）**（SM-3/4；`PLUGIN_PROTOCOL_SUPPORTED=1` 常數凍結支援面）。
   ④ legacy 共存（決策⑤）：`<repo>/.muse/hooks.json` 存在且含 memory matcher（`add_memory|edit_memory` 字面）→ exit 0（SM-5——no-op 讓 legacy 全權；deny short-circuit 未證實故必須 no-op 而非依賴攔截序）。
   ⑤ 安全防護（A7）：`<repo>/.agents/memory-inbox` 逐段 lstat——任何 symlink component → deny＋reason（不落地）（SM-12）。
   ...
   ⑦ **fail-closed 反轉**：governed 路徑（③判定 valid 後）任何內部故障（jq 失敗/寫入失敗）→ deny＋reason，**不再 exit 0**（與 legacy 的 fail-open 分歧點——legacy 靠「未治理即放行」語義，plugin 在治理宣告後禁 fail-open）。
3. **legacy wrapper 過渡**：`hooks/muse_memory_inbox.sh` 改為薄 wrapper（exec plugin 腳本）——單一邏輯源，遷移窗零 drift；S4 cutover 後刪 wrapper。wrapper 行為＝plugin 腳本在 legacy 註冊下的行為（repo 解析對 wrapper 場景＝script所在 repo——plugin 腳本的 git resolution 從 $PWD 出發恰同）。
4. **manifest**：nested `.muse-plugin/`（exactly-one 規則）；hook 欄位帶 `timeout_ms`（防 hang 拖垮 session；deny 需同步結果——**禁 `async`**）。

### Pseudo Code

```
# muse_memory_governance.sh（決策序骨架）
IN=$(cat); [ -n "$IN" ] || exit 0
TOOL=$(jq -r '.tool_name // empty' <<<"$IN" || true)
case "$TOOL" in add_memory|edit_memory) ;; *) exit 0 ;; esac
REPO=$(resolve_repo "$IN")          # stdin workspace 欄位候選序 → git rev-parse；空→exit 0
ST=$(marker_state "$REPO")          # absent|valid|invalid
[ "$ST" = absent ] && exit 0
[ "$ST" = invalid ] && deny "memory-governance marker malformed/unsupported: <repo>"   # 不落地
legacy_matcher_present "$REPO" && exit 0
inbox_safe "$REPO" || deny "inbox path fails symlink/containment check"                 # 不落地
PAYLOAD=$(cas_enrich "$IN" "$REPO")     # legacy lexical gate 逐條（hooks/muse_memory_inbox.sh:35-58）
...
- **關鍵情境**：SM-1/2/3/4/5/8/10/11/12 逐條對應測試（三態矩陣、matcher 場景、subdir、symlink inbox、CAS、atomic、umask、非 .md path lexical gate 繼承）；legacy wrapper 往返一測；**fail-closed 反轉**（jq 以壞 PATH 模擬 → governed deny / ungoverned exit 0 對照）。
- **已知未覆蓋**：live muse 行為（A1/A2/A3）——S2 範圍；overhead——S3 範圍。
...
1. **P-WS（stdin workspace 欄位）**：儀器化腳本 dump stdin key 結構（muse direct＋bridge 兩形態各取樣）；凍結欄位名與優先序回寫 S1 腳本常數。
...
3. **P-ACT-B（bridge headless）**：ai-rules workspace（此時暫不掛 marker——用 probe 子目錄 repo 或暫時性 marker，避開污染）以 bridge `task --family muse` 工單指示呼叫 add_memory → 觀察 receipt；結論同時回答 A2（registry 讀取）。
...
1. **P-UPGRADE**：install v1（content A）→ approve → bump content（v2）→ reinstall → live 呼叫：fire＝approve 續存；不 fire＝**重置形態成立**→ 交付 health check：`muse-memory-governance` 安裝程序（README）＋`hooks/verify-memory-topology.sh --smoke` 擴一節「plugin approve 狀態檢查」（`muse plugins list` trust 欄位）——setup health check 語義入 S5 文檔。
...
3. **收斂手段預留**（超標時先試再 fallback）：早退路徑的 jq 依賴延後（tool_name 判定改純 bash 字串比對，jq 僅 governed 路徑使用）。
...
1. **步驟序**（順序即防護）：plugin install（user-scope）→ approve → live fire 驗證（臨時 marker probe repo，非 ai-rules）→ ai-rules commit marker（`.agents/memory-governance.json`，`protocol:1`）→ ai-rules live 驗證導流 → 刪 machine-local `.muse/hooks.json` → 刪 `hooks/setup-muse-hooks.sh`＋`hooks/muse_memory_inbox.sh` wrapper＋其 legacy 測試（併入 plugin 測試）→ `verify-memory-topology.sh --smoke` 改指 plugin 腳本並跑綠。
2. **marker 進版控**（ai-rules 是 source home——marker 屬 repo 治理宣告，非 machine-local）。
...
4. Cleanup：delegate-bridge `.agent-tmp/muse-plugin-probe/`（跨 repo 刪除由主 session 執行——FINDINGS 紀律）；任務家 `poc/` 依 must-execute 規則處置（行為固化進測試後刪，量測文件保留則結案註記）。
5. 收尾三件＋`/audit-test`＋`/consistency`＋AIR-79 結案兩步＋弧結案蒸餾（預期「無相關 memory 條目」明示）。

### Pseudo Code（修改要點）

```
AGENTS.md:108 段重寫; memory-audit:78 承載句置換
hooks/AGENTS.md + MULTI-MACHINE.md 條目置換
rg 殘留掃描 → 0; /consistency; /audit-test; kanban 結案兩步
```
...
- **形態**：薄 per-repo registration＋單一共享 hook artifact——per-repo `.muse/hooks.json` 指向**同一份** plugin 腳本（穩定 machine 路徑，安裝程序決定：plugin cache 路徑受 content-sha 漂移影響，改用固定安裝點如 `~/.local/share/muse-memory-governance/`——fallback 時定案）；drift 面＝「N 份薄接線」。
