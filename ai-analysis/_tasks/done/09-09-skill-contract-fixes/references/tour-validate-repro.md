...
- **閉環（2026-09-09 晚）**：code-reality v0.6.8 根修發行（真兇＝`strip_prefix` CurDir 組件不匹配——glob 0.3.4 能匹配 `./` 前綴，initial「中文目錄掃描」與「glob 不吃 ./」兩版歸因皆翻案）；release binary 實跑 mosaic 真實語料雙形態 byte-identical（`--repo .` ≡ `--repo /abs`，205 tours fails=0）；post-build SKILL 禁令註記已解除（本機 ≥0.6.8 驗證後）。
