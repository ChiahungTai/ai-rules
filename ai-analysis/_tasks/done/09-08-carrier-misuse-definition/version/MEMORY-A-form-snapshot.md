...
- [feedback_freeze-peripheral-focus-core](feedback_freeze-peripheral-focus-core.md) - 當你要把 AI 發現的周邊 trade-off 升級成主線 blocker 時——user 會以「現在可用就好」凍結爭點拉回核心命題，勿讓次要優化辯論擋主軸
- [feedback_acceptance-evidence-case-alignment](feedback_acceptance-evidence-case-alignment.md) - 當你要補走查/驗收證據（尤其 no-op 處置案例）時——證據三要件：案例實際內容機械複測＋該案例結果對帳（global 指標不頂替）＋如實標等級
...
- [feedback_shorthand-ambiguity-align-first](feedback_shorthand-ambiguity-align-first.md) - 當你要轉述或執行跨 session 工單中的簡稱時——撞詞雙解先對齊所指再動手（writing 案例：muse 與 AI 讀成
...
- [feedback_task-state-lives-in-ep-not-memory](feedback_task-state-lives-in-ep-not-memory.md) - 跟進指派/任務狀態→EP 進度結算節不進 memory；一弧一條=過度建立（29 條 22 終態躺平）；寫前沒載 skill 就動筆是失敗形
...
- [project_session-id-continuation-absorption-0908](project_session-id-continuation-absorption-0908.md) - 當你要跨 harness 接續 session／規劃 muse-plugin-cc 雙家族 bridge 時——--session-id 弧完結；雙家族＝AIR-47；勿以 ROI 勸退
- [project_agents-registry-split-design](project_agents-registry-split-design.md) - 當你要 dispatch subagent／派發代理、查 registry 快照語義或委派 external runtime 時讀——AIR-13/28/29 終態與勿重辯
- [project_memory-cc-alignment-diagnosis-0905](project_memory-cc-alignment-diagnosis-0905.md) - memory 治理線終態——AIR-25 對齊矯正＋三件套/symlink 拓撲/治理教訓；併 AIR-37 卡閘門錨＋AIR-40-42 telemetry 語義（09-08 收斂波）
- [project_cr-live-faces-roadmap](project_cr-live-faces-roadmap.md) - 當你要 query code-reality 符號圖譜／symbol query，或重辯 CR 三弧（SCIP/換軌/pyrefly）定案時讀——勿重辯＋觸發載體定律
...
- [project_memory-redesign-read-path-0908](project_memory-redesign-read-path-0908.md) - AIR-45 終態——(a)採B＋muse 變體＋desc 文法五條落地；dogfood 後續（B 切換/定義表/濫用鑑識）＝AIR-48
...
- [reference_cross-harness-session-continuation](reference_cross-harness-session-continuation.md) - 當你要跨 harness 定向接續 session（resume/fork）時——三家語義矩陣＋store 路徑＋jq 配方＋live/workspace 守衛
