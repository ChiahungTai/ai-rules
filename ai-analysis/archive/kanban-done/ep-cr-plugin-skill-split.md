# [tag:code-reality] CR plugin skill 分層拆遷 EP 追蹤
...
- S1 handoff 交付＋CR 回執（bump 版本＋rg 證據）
- S2 修剪後殘留掃 0 命中＋接線層完整＋cr-dist 管轄行未動
- S3 變體掃全綠（僅允許項殘留）
- S4 `/consistency` gate 綠＋commit（user 授權後）

## 備註

- S3（CRG 殘留 sync）與 S1/S2 無依賴可先行
- 前置已修未 commit：arch-thinking＋illustrate-artifact-menu（同一殘留故事線）
