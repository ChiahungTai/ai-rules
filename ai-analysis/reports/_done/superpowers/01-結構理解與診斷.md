...
> sp 借鑒到 ai-rules 見 [`02-sp借鑒到ai-rules.md`](02-sp借鑒到ai-rules.md)；OpenCode 退路見 [`03-OpenCode退路.md`](03-OpenCode退路.md)。
...
| 2 | **references/ 缺檔 ×3**（tool mapping） | upstream | `gemini-tools.md`（被 `GEMINI.md:2` `@`-include）、`claude-code-tools.md`、`copilot-tools.md` 被 `docs/porting-to-a-new-harness.md` Appendix A / `RELEASE-NOTES.md` 引用，但 references/ 實際只有 antigravity/codex/pi 三檔 → Shape A/C 的 tool mapping 指向斷裂 |
| 2b | **Codex hook 註冊檔 doc-code drift** | upstream | `docs/porting-to-a-new-harness.md` Part 5/Appendix A 多處宣稱 Codex 用 `hooks/hooks-codex.json`（類比 `hooks/hooks.json`/`hooks-cursor.json`），但該檔**不存在**。Codex 實際靠 `hooks/session-start-codex`（存在，被 `tests/hooks/test-session-start.sh:7` 測試）+ `scripts/sync-to-codex-plugin.sh` 整樹 rsync 到 fork repo 運作。doc 描述的「manifest `hooks` 欄位 + hooks-*.json 註冊」機制對 Codex 不成立（`.codex-plugin/plugin.json` 也無 `hooks` 欄位）→ porting doc 對 Codex Shape A 的描述與實作不符 |
...
> 接續：sp 借鑒到 ai-rules 見 [`02-sp借鑒到ai-rules.md`](02-sp借鑒到ai-rules.md)；OpenCode 退路見 [`03-OpenCode退路.md`](03-OpenCode退路.md)；決策摘要見 [`README.md`](README.md)。
