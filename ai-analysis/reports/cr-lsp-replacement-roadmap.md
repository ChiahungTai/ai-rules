...
| **P3 ✅** | **DONE 2026-08-28**：server 本體隨 rebase 退役（三 worktree `tools/lsp_mcp` 歸零）；`.mcp.json` 死條目清除三處 commit（`3040216b`〔main＋v2 同 parent 同內容同秒＝同一 git 物件，正常〕＋`b728b641`〔trading_lab——保留 code-reality 面向〕）；ZCode user-level `lsp-python` entry 已移除（ai-rules 端，config 備份 `.bak-p3`）；:8000 早已無 listener | mosaic＋ai-rules | **取代路線主體完成**——餘 W6（觸發式）與發版 bump 兩個非急件。殘留觀察 ✅ 已清（mosaic `.githooks/post-commit` 純 CRG hook 三 worktree 刪除＋`core.hooksPath` unset——`1c5ae64c`/`c9d032a0`；commit 輸出實證乾淨，死工具連帶 hook 鏈全斷） |
| **分發軸（貫穿）** | plugin＝純 MCP carrier：`.claude-plugin` manifest 補（CC 端一條安裝）＋**Codex 納入**（plugin 承載——機制存在性由 CR 發版時驗證）＋**OpenCode 明示排除**（暫不建，未來需要再議 :8200 共享面）；version bump 紀律、skill 不入 bundle（findings #4 已裁）。**binary 新舊治理 ✅ `929420f`**（rev 嵌入＋--version 帶 rev＋雙信號 stale WARN＋CR repo post-commit 自動重裝 hook 已啟用——殺 silent install-lag 類，2026-08-28 三中；mcp bin 的 version 面 `89ff971` 補齊——freshness「任一 bin」宣稱自此為真）。**發版 bump ✅ `0827594`＝0.1.2——分發軸全閉**：bump→ZCode cache 0.1.2 刷新實證（findings #6/Validate #3 關帳）→user-level 手動 entry 移除→**plugin entry 掛載雙 session 電池實證**（ai-rules＋mosaic 各三項全過——工具前綴 `mcp__plugin_...` 證明走 plugin 路徑） | CR | ~~分發軸殘餘~~ 全清 |
...
| W5 ✅ | **DONE 2026-08-28（CR `4a36b22`，22 檔 +273/−1,445；post-commit hook 自動重裝驗證過——三 bin `0.1.0+4a36b22`）**：`import_legacy` 零殘留（活面 rg 全零、`-h` 只剩 `build\|ensure_indexes`、op→exit 2 loud 拒絕）；CR 自身舊庫**先重建後刪**（rust 鏈 899 nodes/2,277 edges→spot-check→刪 7.9M——never-delete-first 遵守）；**撈到兩個真 bug**：F-A `scip_refs --audit` wrapper 殘留舊 CRG 路徑（cutover 漏網回歸，修後 exit 0 缺差 0）＋F-B freshness WARN 不標主體（unscoped diagnostic——修為自指訊息）；雙軸審查零 🔴🟡；遞延項已落地（`f82b274`：fixture rename＋bridge flake starvation-gate retry——全 workspace 41 suites 並行負載全綠）；另 `89ff971` 補 mcp `--version`/`--help` 面（原成員測試式 parse 讓未知 flag 靜默落 HTTP 常駐） | CR 小弧 | **資料面自理軸 W1-W5 全終結** |
| W6 ✅ | **DONE 2026-08-28——評估已足、user 裁決不建**（雙向門：數字留檔可重評）。評估弧實證：**Rust/NT 高信心恢復池 ≈0-1 pairs**（四輪除噪——字串 off-by-one／fn 宣告自匹配／std 撞名 join；232k 邊基數；RA 1.97.1→1.98.0 零新增 attribution）；**Python/mosaic 假前提修正**——`py_calls` 非獨立語法 producer（ruff 只分類 pyrefly reference rows），獨立 AST 掃描 name-form 淨值 ~70-80 pairs（0.3%）、getattr 雙面全盲 ~47 sites。否決理由＝第二層後果不成比例（95.42% gate 基準失效＋hub/communities 漂移＋NT byte-identical 治理鉤子破裂→全下游 re-baseline vs 0.3-0.5% 精度未知）。報告＝`~/.mosaic/code-reality/w6-scratch/`（裁決後清）。**附帶認知修正**：RA SCIP 幾乎不輸出 std method occurrences（「blind」大宗＝正確略過）；19.8% 舊數字屬量測 artifact 疊加非真互補面 | 評估弧（CR session） | **取代路線全線終結** |
...
**Demand-pull（需求拉動，2026-08-28 增補——中央規劃的補充軌）**：加值型工具弧（如 W6）由消費端觸發而非 hub/CR 猜測——①消費端 session 在**自己的** `.kanban/Backlog/` 開 `[cr-demand]` 卡（觸發場景＋實證缺口＋期望能力——單一寫入者拓撲合規）；②ai-rules hub 每次 relay 收尾掃三家 `[cr-demand]` → 本檔需求收件匣 → user 裁決排程 → kickoff。先例＝mosaic pipeline-gap 報告→`2442692`（無形版流程，此為正式化）。模式參照 open-source issue tracker（消費端 file、維護方 triage）／RFC（設計級變更由需求方提案）。
