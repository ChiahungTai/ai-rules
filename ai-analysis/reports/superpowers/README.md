...
| [`04-multi-harness機制對照.md`](04-multi-harness機制對照.md) | sp vs ai-rules multi-harness 機制對照；轉型後回頭檢驗 `02`「不借結構類」；下游缺口（skills/commands/載入語意/acceptance test）；**§7 補 ZCode 權限模型實測**（3.2.5 hooks 失效紀錄 + **3.7.7 翻案：user-level config hooks 實測可用**，stdin 含 Claude snake_case alias；ai-rules 場景採 yolo + 人工把關）| multi-harness 機制是否要借、ZCode 權限決策時 | ai-rules 轉型 multi-harness 後、評估機制借鑒時、用 ZCode 遇權限彈窗問題時 |
