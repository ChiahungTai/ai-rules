...
- 實測（07-04 log）：config 含 hooks 區塊時，ZCode 載入並觸發 hook（38 個 `hook.run.failed` 事件，source 標示 `config.PreToolUse.0.0` / `config.Stop.0.0` / `config.PermissionRequest.0.0`），但**全部執行失敗**（`message: "Hook execution failed"`）；07-05 config 無 hooks 區塊 → 0 個事件。
...
- **LSP**：ai-rules 純文檔 repo（3 個獨立 .py 腳本無跨檔依賴），LSP 無價值；跨 harness LSP 載體結論見 item 4
...
4. **LSP 跨 harness 無統一載體**：各家 harness 有自己的 LSP 機制——Claude/OpenCode 原生內建（Claude 12-lang LSP plugin set：pyright/rust-analyzer/clangd/gopls/jdtls/...）；ZCode 無原生 → mosaic_alpha 自建 `lsp-python` MCP server（http，per-project，`tools/lsp_mcp/server.py`）替代。**沒有「一個 MCP server 通吃」的通用解**——Claude/OpenCode 用原生反而更簡單，ZCode 需自建 MCP server 替代。mosaic_alpha 的 `lsp-python` 是 ZCode 端已實作的替代方案（per-project workspace + 可控 7 tool：hover/definition/references/diagnostics/rename_symbol/edit_file + reload_workspace），**非跨 harness 通用解**。
