...
- **Muse memory（受約束寫入者——AIR-54）**：記憶主體＝repo 內 `.agents/memory/`（gitignored；自帶池 git）——muse 以 project scope 原生讀（`read_memory`；開場自動注入 MEMORY.md），CC 端 `~/.claude/projects/-Users-ctai-Github-ai-rules/memory` 換目錄 symlink 指主體、ZCode 既有鏈雙跳；寫入流：`add_memory`/`edit_memory` 被 `.muse/hooks.json` PreToolUse 閘（`hooks/muse_memory_inbox.sh`；hooks.json 為 machine-local 生成物，`hooks/setup-muse-hooks.sh` 產生，clone 後重跑）代存 `.agents/memory-inbox/` 並 deny——**禁繞過 inbox 直寫條目或索引**；consolidation（六問＋frontmatter＋path contract/CAS）＝memory-audit skill「Inbox 消費」節（夜波——波前二分之後；或手動）
