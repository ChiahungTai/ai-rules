...
- delegate-bridge — **雙家族委派 bridge** plugin（muse＋codex，`task --family muse|codex`；ZCode/CC 雙端 marketplace 發佈），**住獨立 repo `~/Github/delegate-bridge`（非本 repo 目錄；原 muse-plugin-cc 搬遷更名，v1.0.0 起）**；消費形態 plugin agent（`delegate-rescue` 委派、`delegate:delegate-runtime` skills）＋ bridge CLI；計費鐵則＝muse 純訂閱 5h 窗口（無 API key 面）、codex ChatGPT 訂閱載具（bridge 剝 `OPENAI_API_KEY` 防 API-key 計費翻轉）；預設模型 pin 與 task 預設 effort 現值見 model-routing skill 解析表（單一源，不在此 materialize）；muse CLI 事實真相源 [memory reference_muse-code-cli-facts]、codex raw CLI 事實 [memory reference_codex-cli-raw-facts]、委派細節真相源 repo `docs/`
