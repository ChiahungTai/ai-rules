...
- `agents/` — 跨 harness subagent 定義（registry 結構：`shared/` 內容切片＋`zcode/`／`claude/` per-harness registry 視圖，`~/.zcode/agents`→`agents/zcode/`、`~/.claude/agents`→`agents/claude/`；tier-pinned 家族〔lite-verify/spec-miner/vision-review/cr-research/mem-distill〕、欄位相容策略與 ZCode Beta 限制見 [agents/AGENTS.md](agents/AGENTS.md)）
