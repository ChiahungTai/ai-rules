...
- **Muse memory 唯讀**：`muse` 在本 repo **只讀不寫**——可用 `read_memory`（三 scope）與 `read_file` fallback 到 CC 實體池 `~/.claude/projects/-Users-ctai-Github-ai-rules/memory/`，**禁 `add_memory`/`edit_memory`**；需固化事實由 ZCode/CC 側寫入（見 memory 條目 `memory-cc-alignment-diagnosis-0905` 的共用池 symlink 拓撲段）
