...
> **為什麼兩份一起**：CLAUDE.md（what / where）與 architecture.md（why / whole-picture）同為導航文檔（見 [AGENTS](../AGENTS.md) 文檔體系）。涉及設計變更時兩份都要看，避免 architecture.md 漂移成 LLM 讀不到現狀設計。純 feature（不改設計）只更 CLAUDE.md。
...
> **核心原則**：導航文檔（CLAUDE.md / architecture.md / SYSTEM-MAP.md，見 [AGENTS](../AGENTS.md) 文檔體系）任一份內部 drift 都誤導 LLM。本次修改過的導航文檔必須通過單檔自洽閘門。
