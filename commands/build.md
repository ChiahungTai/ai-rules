...
4. **Examples 盤點**：掃描專案中的 example/demo 檔案（常見模式：`demo_*.py`、`examples/**/*.py`、`scripts/demo_*.py`、`notebooks/*.ipynb`），建立 `{module} → [example paths]` 映射表。後續段落級驗證和整合驗證都以此表為準
5. **檢查清單**：
   - [ ] Examples 盤點完成，映射表已建立
...
4. **Wave 合併**：同一 wave 所有 Agent 完成後，將 Agent worktree 修改應用到主 worktree：
   - Agent 成功且有修改 → 讀取 Agent worktree 產出，確認品質，將變更應用到主 worktree（若 Agent 已 commit → `git merge --squash`；若 uncommitted → 讀取檔案 Edit 到主 worktree）
   - Agent 成功但修改有問題 → 記錄 ⚠️，主 Agent 手動修正所需部分
   - Agent 失敗 → review output 決定是否手動修或跳過
   - 合併後執行 `uv run ruff check --fix . && uv run ruff format .` 確保 Agent 引入的 lint 問題被修正
5. **Wave 間序列**：合併完成 → 整合驗證 → 下一 wave
...
### Examples 映射
- {該段落修改的模組所對應的 example/demo 路徑，或「無相關 example」}
- **強制**：Agent 回報前必須執行至少一個 Example 驗證（`uv run python <path>`）
...
- **Examples 強制驗證**：Agent prompt 包含 Examples 映射時，Agent 必須在回報完成前執行至少一個 Example（`uv run python <path>`），驗證通過才能標記完成
...
- **Examples 驗證**：查映射表，若段落修改的模組被任何 example 引用 → `uv run python <example_path>` 執行驗證
- 確認通過 → 記錄完成 → 下一段
...
4. **Examples 全量驗證**：
   - 基於準備階段的映射表，執行所有 example/demo 檔案
...
- **`/goal`** — 設定完成條件，Claude 跨 turn 持續實作直到條件滿足。啟動 `/build` 後設定：
  ```
  /goal all segments implemented, uv run pytest exits 0, uv run ruff check . is clean, uv run mypy . is clean, all demo scripts run without error
  ```
