...
> **CRG（若裝了）**：Agent Review 的 wiring 驗證（新/改 symbol 的消費端接對沒）用 CRG `query_graph callers_of`；段 impact 驗證用 `get_impact_radius`；「沒影響 X」的 claim 機械反證用 CRG（graph 是 L1 機械證據，補強 Claim→Evidence→Trust，見階段 3 整合路徑檢查）。LSP-vs-CRG 分工 + assume-present + warn-if-absent 見 [crg-query](../skills/crg-query/SKILL.md)。
