...
**觸發條件**（避免過度工程）：任務含 ≥ 5 段都是[中型變更](../AGENTS.md)（變更規模分級見 AGENTS）→ blueprint；否則 implementation。視複雜度調整 — 小型任務硬拆成 blueprint 是過度工程。
...
| 規模 | 判準 | 流程 |
|------|------|------|
| **simple** | 單檔 bug fix、小 tweak、🟢 低風險 | **不寫 EP** — 直接 TDD + 驗證（`/build` 裸任務或直接實作）；對齊 [AGENTS](../AGENTS.md)「小型變更不需 UC」 |
...
**欄位含義**：見 [AGENTS.md](../AGENTS.md) 的「Scenario Matrix」段落
...
1. **可複用基礎設施盤點**：搜尋需求涉及的 CLAUDE.md Capabilities + LSP `workspaceSymbol` 搜尋相關 class/function，找出可複用的 utilities、base classes、protocols
...
**前期 POC**（高風險假設可行性驗證，吸收舊 `/spec` Phase 3）：段落 0 全域研究識別的高風險技術假設（外部 API、SDK 行為、從未用過的函式庫），在此段落以 `poc/poc_*.py` 驗證可行性（能不能做）；深度驗證（效能、邊界、壓力）由 `/ep-validate` 於 EP 後執行。POC 檔頭格式見 [/ep-validate](./ep-validate.md)。
