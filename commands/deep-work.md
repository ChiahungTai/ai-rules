...
- **ARGUMENTS = `/build <EP>`**（observed 主路徑）：流程骨架**委派 [build.md](./build.md)**（階段 0-6 全跑），deep-work 自身階段 1-5 **不執行**。**不得省略的 build 步驟**（無人在場時唯一機械防線）：階段 0「EP 快檢」強制輸出、階段 1「POC + demo 盤點」映射表、**階段 2「整合路徑覆蓋硬閘門」**（`rg "<新參數>=" tests/`）、階段 5c `/audit-test` —— 絕不靜默跳過（`/deep-work /build` 時硬閘門在最需要它的無人路徑必須生效）。
