...
- **組合下不得省略的 build 步驟**（無人在場時唯一機械防線）：階段 0「EP 快檢」強制輸出、階段 1「Examples 盤點」映射表、**階段 2「整合路徑覆蓋硬閘門」**（`rg "<新參數>=" tests/`）、階段 5d `/audit-test`。絕不靜默跳過。
...
**Writer/Reviewer 分離**：用戶不在場，Agent Review 是唯一的品質閘門。2-perspective review（① Intent-anchored + ② Fresh），完整流程見 [agent-review-cycle.md](./claude/_common/agent-review-cycle.md)。
