---
description: "進度儀表板。直接讀取 .kanban/ 目錄和模組 Capabilities（AGENTS.md 為主，CLAUDE.md legacy fallback），產出進度報告。"
...
> **角色**：Kanban 的 view layer — 把 .kanban/ 目錄結構 + 模組 instruction 檔（AGENTS.md 為主，CLAUDE.md legacy）Capabilities 翻譯成人可讀的進度表。回答「做到哪了？完成率多少？有沒有過時的卡片？」
...
| 模組 instruction 檔（AGENTS.md 為主，CLAUDE.md legacy） | `fd` + `Read` | `## Capabilities` 表格的 ✅ 條目 |
...
| 3 | 讀取 Capabilities | `fd "AGENTS.md" <package>/ --type f`（legacy 單檔模組 fallback `fd "CLAUDE.md"`），解析 `## Capabilities` 表格的 ✅ 數量 |
| 4 | 偵測過時卡片 | 檔案 mtime：Backlog > 30 天、In-Progress > 14 天、Next-Up > 7 天 |
| 5 | 聚合統計 | 按模組（tag）聚合 Kanban 卡片 + 按 instruction 檔模組聚合 ✅ |
...
- 掃描每個模組的 instruction 檔：`{project_root}/**/AGENTS.md`（雙檔模式內容來源）；legacy 單檔模組 fallback `**/CLAUDE.md`
- 搜尋 `## Capabilities` 標題下的 markdown table
- 計算每個模組的 ✅ 條目數
- 模組 = instruction 檔所在目錄名
...
- 掃描路徑：`<package>/**/AGENTS.md`（遞迴 glob；legacy 單檔模組 `**/CLAUDE.md`）
