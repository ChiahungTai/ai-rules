...
**關鍵新增**：避免將「概念範例」誤認為「實際實作」

#### **分析重點**
- **文檔結構理解**：區分「概念說明」vs「具體實作」vs「增強設計」
- **設計意圖識別**：理解每個代碼片段的目的和上下文
- **段落間關係**：分析不同段落間的邏輯關係和依賴
- **重複代碼的合理性**：判斷重複是否為不同用途的合理設計

#### **具體任務**
- 檢查標識語言：如「範例」、「實作」、「概念」等關鍵詞
- 分析段落標題和說明文字
- 識別教學性代碼 vs 生產性代碼
- 判斷設計模式的重複是否合理
...
**文檔脈絡分析結果應用**：
分析結果將影響後續審查策略的選擇和執行重點，確保不同類型的文檔使用適當的審查標準。
...
##### 1. 文檔脈絡信心度（權重 30%）
- **100分**：確認是真實的 runtime 錯誤或邏輯缺陷
- **90分**：很可能是實際問題，但需要更多上下文確認
- **70分**：可能是問題，但需要考慮設計意圖
- **50分**：表面看像問題，但可能是合理設計選擇
- **30分**：很可能是 False Positive，需要深度分析
- **0分**：確認是 False Positive
...
#### 最終評分公式
最終信心分數 = (文檔脈絡信心度 × 0.3) + (技術問題信心度 × 0.4) + (實際影響評估 × 0.3)
