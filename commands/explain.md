...
graph TD
    ...
    style D fill:#3b82f6,color:#ffffff  <!-- 僅此一個模式節點 -->
    style H fill:#10b981,color:#ffffff  <!-- 僅此一個處理節點 -->
```
...
#### 階段 1: 檔案分析（並行 Tasks）
當檔案數量 ≥ 5 個時，系統自動啟動並行分析：

```bash
# 同時發起三個獨立的 Task 工具調用
Task 1 (structure-analyzer): 分析檔案結構、依賴關係、模組劃分，生成檔案關聯圖和依賴樹
Task 2 (content-analyzer): 深度解析程式碼邏輯、設計模式，提供核心邏輯說明和決策分析
Task 3 (context-analyzer): 分析 Git 歷史、文件註解、配置，提供開發背景和演進過程
