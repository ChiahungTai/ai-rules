...
Workflow 審查協調：[workflow-review-pattern.md](claude/_common/workflow-review-pattern.md)（Ultracode 下平行四維度審查）
...
Workflow 完成後回傳 `{confirmed, stats}` → Main LLM 合成 4 個 DimensionVerdict → 執行回寫（回寫原則見下方）。

印出確認：`[EP Review Mode] effort=ultracode, workflow=true, max=N`

**直接審查模式**（Fallback，max-agents = 1 或非 ultracode）：

單一 Explore agent 做所有 4 維度（現有行為）。印出確認：`[EP Review Mode] effort=standard, workflow=false`
