...
> **受眾**：LLM 執行鏈命令（layer 1 同 session 自判 / layer 2 跨 session），產出回寫 EP 餵回產 EP 的 LLM，**不給人看**。EP 的人類 viewport 是 `/human-review --ep`（layer 3，4 lens）。見 [CLAUDE.md](../CLAUDE.md)「命令的受眾視角」。
...
Workflow 審查協調：[workflow-review-pattern.md](./claude/_common/workflow-review-pattern.md)（Ultracode 下平行五維度審查）
...
**Workflow 模式**（effort = ultracode/xhigh 且 max-agents > 1）：
...
**直接審查模式**（Fallback，max-agents = 1 或非 ultracode）：

單一 Explore agent 做所有 5 維度（現有行為）。印出確認：`[EP Review Mode] effort=standard, workflow=false`
