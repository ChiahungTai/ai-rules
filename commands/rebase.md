...
usage: "/rebase <trunk-branch>"
argument-hint: "<trunk-branch>（慣例 main）"
allowed-tools:
  - Bash
  - Read
  - Edit
...
Feature branch rebase onto **trunk**（慣例 `main`）。多 worktree 共用同一 `.git`，rebase 即時本地、不需 push。本命令只處理「feature onto trunk」方向 —— trunk 永不被 rebase（見鐵律）。
...
0. **Trunk 守門（雙向）**：
   - (a) `git branch --show-current` 若是 trunk（`main`）→ **拒絕**（鐵律 2：trunk 永不被 rebase），改在 trunk worktree 跑 `git merge --ff-only <feature>`（鐵律 4）。
   - (b) 輸入 `<trunk-branch>` 必須是 trunk —— feature 之間**不互 rebase**（鐵律 3）。`backbone rebase replay` 之類要拒絕，改走 trunk 中轉（replay→main→backbone）。
...
```bash
git merge-base --is-ancestor HEAD <trunk-branch>   # HEAD 是 trunk 祖先 → fast-forward
git merge-base --is-ancestor <trunk-branch> HEAD    # trunk 是 HEAD 祖先 → up-to-date
# 兩者都不是 → diverged
git log --oneline HEAD..<trunk-branch> | wc -l      # trunk 獨有 commit 數
git log --oneline <trunk-branch>..HEAD | wc -l      # current 獨有 commit 數（將被 replay）
```
...
**基本規則**：當前（feature）branch rebase onto 輸入的 trunk branch。

| 結果 | 處理 |
|------|------|
| 成功 | `✅ <current> rebase onto <trunk-branch>（N commits replayed）` |
| 衝突 | **進入衝突分析流程**（見下方） |
...
| Block | 行號 | HEAD（<trunk-branch>）加/改 | Incoming（<current>）加/改 | 衝突類型 | 建議 |
|-------|------|----------------------|--------------------------|---------|------|
...
2. 對每個**非 trunk** 的 feature worktree：先 `git -C <path> status --porcelain` 確認 clean
3. **Rebase onto trunk**：`git -C <path> rebase <trunk-branch>`（不是 rebase 彼此，鐵律 3）
...
| 參數 | 說明 |
|------|------|
| **\<trunk-branch\>** | 要 rebase onto 的 trunk 分支（慣例 `main`；feature 之間不互 rebase） |
...
- 同步其他 feature worktree 前，每個必須先確認 clean
...
```
/commit（--push / --rebase）→ /rebase <trunk>（feature onto trunk，可選同步其他 feature）
```
