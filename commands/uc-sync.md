...
| **品質層** | + 格式合規 + 重複偵測 + 章節歸位 + 消費場景欄位 + Test Coverage | `--quality` |
| **完整層** | + 覆蓋缺口 + 過時項目 + SYSTEM-MAP 一致性 | `--all` |
...
| 3 | 跨引用完整性 | 核心 | Domain UC 的跨域引用（領域依賴、下游消費者）是否存在？孤立 UC ID？ |
...
| 11 | Test Coverage | 品質 | ✅ UC 是否有對應 test 保護？ |
| 12 | SYSTEM-MAP 一致性 | 完整 | SYSTEM-MAP 功能的 UC ID 引用是否存在？生命週期狀態是否與底層 UC 一致？ |
...
**核心原則**：UC 的跨域引用必須存在，UC ID 不應孤立。

| 檢查項 | 嚴重程度 | 驗證方式 |
|--------|---------|---------|
| Domain UC 的「領域依賴」引用的 UC ID 不存在 | Critical | 引用了不存在的 UC |
| Domain UC 的「下游消費者」引用的 UC ID 不存在 | Important | 可能是改名或刪除後未更新 |
...
| UC 不在主要實作模組目錄 | Important | 應放在主要實作所在的 library 目錄 |
| UC ID 前綴與所在模組不符 | Important | 如 SJ-* 前綴的 UC 出現在 venues/ |
...
| ✅ UC 路徑未使用 markdown link 格式（應為 `` [`file.py`](/path/to/file.py) ``） | Suggestion |
...
### 角度 11：Test Coverage（品質）

**核心原則**：✅ UC 應有測試保護，否則重構時可能靜默破壞。偵測為啟發式（目錄匹配 + 檔名匹配），需人類確認。

| 檢查項 | 嚴重程度 | 驗證方式 |
|--------|---------|---------|
| ✅ UC 無對應 test 檔案 | Suggestion | 搜尋 `tests/unit_tests/{domain}/` 或 `tests/**/test_{module}.py` |
| ✅ UC 標記 `testing: N/A` 但有對應 test | Suggestion | 確認豁免是否仍合理 |
| ✅ UC 標記 `testing: N/A` 且無對應 test | — | 豁免項，不報告 |

### 角度 12：SYSTEM-MAP 一致性（完整）

**核心原則**：SYSTEM-MAP.md 的功能狀態應反映底層 UC 的真實狀態。SYSTEM-MAP 是人類的第一個文件，不準確的狀態會誤導決策。

**前置條件**：專案根目錄存在 `SYSTEM-MAP.md`。不存在時跳過此角度。

| 檢查項 | 嚴重程度 | 驗證方式 |
|--------|---------|---------|
| SYSTEM-MAP 中引用的 UC ID 在 USE-CASES.md 中不存在 | Critical | 提取 SYSTEM-MAP 中的 UC ID，逐一比對 USE-CASES.md |
| 功能標記 ✅🔍 但底層 UC 有 📋/🔧 | Important | 聚合功能內所有 UC 狀態，比對功能 life-cycle |
| 功能標記 🏃 但底層 UC 有 ❌ 或大量 📋 | Important | 同上 |
| SYSTEM-MAP 中存在功能但無對應 UC | Suggestion | 功能描述無法對應到任何 UC ID |
| UC 狀態全部 ✅ 但功能仍標 ⚠️ | Suggestion | 可能是過時的 ⚠️ 標記，建議人類確認 |
...
| 11 | Test Coverage 檢查 | `--quality`（角度 11） |
| 12 | 覆蓋缺口掃描 | `--all`（角度 7） |
| 13 | 過時項目掃描 | `--all`（角度 9） |
| 14 | SYSTEM-MAP 一致性 | `--all`（角度 12） |
| 15 | 產出報告 | 預設 |
...
### 步驟 1.5：Snapshot 消費（如果可用）

> **觸發條件**：專案根目錄存在 `.project-snapshot.json` 時執行。無 snapshot 時跳過。

如果 `.project-snapshot.json` 存在（由 `/scan-project` 或 `/claude:daily-maintain` Phase 1 產出），直接消費預計算資料：

- `uc_registry[]` — 所有 UC 條目（ID、狀態、路徑、交叉引用）→ 取代步驟 2 的手動解析
- `uc_edges[]` — UC→UC 依賴邊 → 取代步驟 5 的手動追蹤
- `cross_validation[]` — 預計算的 X7（斷裂引用）、X-unique（重複 ID）、X-path（路徑失效）→ 直接併入報告

**降級邏輯**：snapshot 的 `uc_registry` 可能比實際 USE-CASES.md 舊。使用前檢查 `scan_timestamp`，如果超過 24 小時建議先執行 `/scan-project` 更新。

### 步驟 2：解析所有 UC 條目

**有 snapshot** → 直接使用 `uc_registry[]`，跳過手動解析。
**無 snapshot** → 完整讀取每個 USE-CASES.md，提取：
...
對每個 UC 條目的路徑（**格式判斷**：純文字路徑直接用；markdown link `[text](url)` 提取 `()` 內 href）：
...
1. 收集所有 UC 的「領域依賴」和「下游消費者」欄位中的 UC ID
2. 比對引用集 vs 定義集，找出懸空引用和孤立定義

### 步驟 6：驗證 Domain-First 合規

1. 確認掃描範圍內無 scripts/ 下的 USE-CASES.md
2. 確認 UC ID 前綴與所在模組目錄一致
...
### 步驟 11：Test Coverage 檢查

對每個 ✅ UC，使用啟發式策略檢查是否有對應 test：

1. **目錄匹配**：從 UC 路徑提取 domain，搜尋 `tests/unit_tests/{domain}/` 或 `tests/{domain}/`
2. **檔名匹配**：從 UC 路徑提取模組名，搜尋 `tests/**/test_{module}.py`
3. **豁免檢查**：UC 條目含 `testing: N/A` 或 `testing: manual` 時標記為已豁免

找不到匹配且無豁免 → 報告為 Suggestion 級別問題。
...
- [ ] **跨引用斷裂**: D-14 引用 D-99，但 D-99 不存在於任何 USE-CASES.md

### 🟡 Important（建議修正）

- [ ] **狀態過時**: 📋 D-31 已有實作程式碼，建議升級為 ✅ 或 🔧
- [ ] **Domain-First 違規**: UC ID 前綴與所在目錄不符（如 D-XX 定義在 features/USE-CASES.md）
...
- **只讀不寫，低風險例外**：本命令預設只檢查和報告。但以下問題修正成本極低且無歧義，發現時直接修正不詢問：路徑引用指向已更名/遷移的檔案（且能明確對應到正確檔案）。只有涉及模糊匹配（多個候選）或語意變更（描述文字要改）才需要確認
