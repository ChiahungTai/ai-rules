...
when_to_use: "Genius-engineer-shows-boss framing: render completed work for acceptance, force the AI to point at concrete demos (not just claim 'done'). Product-type-aware — code-mode renders a demo checklist (feature → runnable demo → run → confirm); docs/rules-mode renders behavior delta (what AI does differently) + affected UCs. --ep renders PLANNED deliverables (UC/scenario + planned demo targets) for pre-build direction confirmation. Surfaces cognitive-gap points. Priority: direction >> quality; does NOT audit structure (/arch-review) or correctness (/code-review)."
...
**不做的事**：逐行正確性 → `/code-review`；結構 / 重用 → `/arch-review`；跑測試 / 覆蓋率 → `/build` + `/audit-test`。
...
arch-review 新增              審查渲染結構拓樸 + 重用枚舉                 post-EP/build 結構審查
...
1. 校正方向偏差 / 補漏 demo
2. → /arch-review（結構）/ /code-review（正確性）→ /commit
```
...
| 案例 | 處理 |
|------|------|
| 純重構（無行為變更、無新 feature） | 標「無交付變更，不適用」→ 建議 `/arch-review` |
