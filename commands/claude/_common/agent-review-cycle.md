...
### 為何同時兩個（bias ↔ coverage 正交）

兩 lens 抓的盲點**正交**，單跑任一有盲點，同時跑才互補：
...
單跑 clean → 漏該查的 UC；單跑 UC-anchored → 被錨定看不到合理化。兩者盲點不重疊（正交），同時跑覆蓋最廣 —— 這是「固定 2 個」而非單一或 N 個同錨定 agent 的根據（多樣性 > 數量）。

### 為什麼固定 2 個（不再 adaptive 多維度 agent）

- **多樣性 > 數量**：舊設計的 4 個 dimension agent（正確性 / 架構 / 測試覆蓋 / EP 合規）共享錨定，邊際覆蓋遞減。2 個異質 perspective（clean + UC-anchored）正交覆蓋更廣。
- **機械軸不靠 agent**：測試路徑覆蓋（新參數 `rg "<param>=" tests/`）是 `/build` 階段 2 硬閘門 + 階段 5d `/audit-test`，不靠 review agent 重複。正確性 / 架構由 clean agent 讀 code 自然覆蓋；EP 合規由 UC-anchored agent 覆蓋。
...
| 條件 | 模式 |
|------|------|
| max-agents ≥ 2 | **① clean + ② UC-anchored** 兩 agent（預設） |
| max-agents = 1 | 單一 agent：UC-anchored（載入 UC / intent）+ 明示「同時 clean-eye 找 UC 之外的問題」（降級 2-in-1） |
...
max-agents > 2 且高風險 / 大變更時開 extra agent。規則（單一源）見 [review-engine](../../../skills/review-engine/SKILL.md)「review 執行預設」>2 配置：base 恆 ① clean + ② UC-anchored；extra（第 3+）優先序到 cap —— **UC 數 >6 → UC-split**（extra 拿 UC 子集做深度）；否則 **architecture > adversarial / edge > consumer-perspective**；**絕不**複製 lens（2nd clean / 2nd 同 UC 同家族共享盲點，邊際 ≈0）。執行範本據此組 prompt。
