...
### >2 配置（機械特徵觸發，非語義 opt-in）

extra agent 由**消費命令提供的段落風險特徵**機械觸發（非 LLM 語義判「高風險」）。映射框架（通用風險特徵 → extra agent）定義在 [review-engine](../../../skills/review-engine/SKILL.md)「review 執行預設」點 5（單一源，此處不重複以免 drift）；base 恆 ① clean + ② UC-anchored。build 的 adapter 接線（build 機械信號 → 通用特徵）見 [build.md](../../build.md) 階段 4「adaptive 觸發映射」。執行範本據此組 prompt。
