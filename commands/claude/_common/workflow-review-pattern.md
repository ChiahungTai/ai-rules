...
> **核心原則**：審查發現(finding)的對話對象主要是另一個 LLM(`/build` 內串接、`/copy` 給外部 LLM)。**跨命令自動化場景**（接 `/judge-review`/`/followup-review`）finding 須持久化、結構化、跨命令可追蹤；**人主導工作流**（review session 對話 + `/copy` 搬運）finding 留對話由人對照 diff 判讀，不強制持久化（見下方「持久化位置（optional）」）。
