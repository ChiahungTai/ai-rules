...
#### **Agent #4: Spec 蒸馏專家**
基於 `/distill-spec.md` 處理 specs/ 目錄：
- 執行蒸餾流程 (`/distill-spec`)
- 解決版本矛盾和重複
- 提煉核心技術決策
- 重組文檔結構

#### **Agent #5: Claude.md 蒸馏專家**
基於 `/distill-claude.md` 處理 CLAUDE.md 檔案：
- 執行蒸餾流程 (`/distill`)
- 提煉核心開發原則
- 去除冗餘實作細節
- 保留 AI 導航資訊

#### **Agent #6: Lessons 提取專家**
基於 `/lessons.md` 處理知識提取：
- 分析對話歷史中的新知識
- 識別未記錄的重要決策
- 提取新的技術規格
- 更新相關文檔
...
#### **Agent #7: INDEX.md 生成專家**
參考 bmad 的 index-docs.xml 生成索引：
- 掃描 specs/ 和 docs/ 目錄
- 按類型和用途分組文檔
- 生成準確的文檔描述
- 建立清晰的導航結構
...
📊 階段 1: 文檔現況分析
📋 Agent #1 - 文檔掃描: 發現 18 個文檔，3 個目錄
🔍 Agent #2 - 品質評估: 檢測到 4 個重複區塊，2 個矛盾
🎯 Agent #3 - 決策分析: 識別 8 個關鍵架構決策

⚗️ 階段 2: 蒸餾與清理
🔧 Agent #4 - Spec 蒸馏: 處理 specs/modules/ (5 個檔案)
   ✓蒸餾完成: -32% 體積，解決 2 個矛盾
🎓 Agent #5 - Claude.md 蒸馏: 處理 3 個 CLAUDE.md
   ✓蒸餾完成: -28% 體積，提煉核心原則
📚 Agent #6 - Lessons 提取: 分析 1000+ 則對話
   ✓新決策: 2 個重要架構選擇
   ✓新規格: 5 項技術要求

📋 階段 3: 索引生成與整理
🗂️ Agent #7 - INDEX 生成: 建立 specs/INDEX.md
   ✓索引條目: 18 個文檔，4 個分類
🔗 Agent #8 - 結構優化: 改善導航和連結
   ✓優化完成: 統一模板，改善連結
