...
只做第一性原理分析，會產生「本質正確但後果災難」的決策（Tesla 生產地獄：製程設計從物理原理出發完全正確，但未追蹤過度自動化在真實工廠環境的連鎖後果）。
...
範例領域特定（mosaic：domain=策略訊號 / use case=回測下單 / adapter=NT·SJ·catalog / infra）。深入視角（三主線在 spec/illustrate/EP/implement 各自怎麼用）與結構機械見 arch-thinking skill（on-demand）。
