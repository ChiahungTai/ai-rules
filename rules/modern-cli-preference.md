...
**符號/圖譜查詢用 code-reality（index 在場；型別面〔hover/diagnostics〕雙語言用 code-reality-lsp-bridge——.py/.rs 副檔路由），文字搜尋用 rg，檔案搜尋用 fd。** `fd`/`rg` 預設遵守 `.gitignore`（減少噪音）。四路路由細節見 [lsp-navigation.md](lsp-navigation.md)。
