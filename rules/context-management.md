...
- **禁含完成度宣稱**（negative guidance + 強度上限（指導非強制）見 [state-md-write](../skills/_common/state-md-write.md)）
...
**觸發**：「離開前寫」綁 session 結束——寫入由 at 排程 / deep-work completion 觸發（Claude 端：`/at`、`/deep-work`；寫入步驟見 [state-md-write](../skills/_common/state-md-write.md)）；handoff 僅加 note 標示「STATE.md 非交接選項」，不寫入。「開場讀」綁 resume。
