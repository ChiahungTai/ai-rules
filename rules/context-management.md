...
寫 memory **前**先跑一句話測試（核心事實提煉不成一句話＝還沒想清楚＝不寫——先想再寫；提煉得出但屬未定案歸因/推測＝還沒確定＝不寫，定案後再放——user 09-06 拍板）＋寫入六問（**任務終態→卡/report 不進 memory**〔user 09-05 拍板——弧歷程/流水/軌跡屬任務，活知識才進 memory〕／repo 可推導就不寫／同主題加段／project 條目先收斂／尺寸預算／載體判定——紀律→rule、方法論→skill、跨 session 事實→memory、承諾/待辦→backlog 卡）＋新條目 rank 初判（hot＝活躍弧/高頻教訓、core＝default、cold＝冷門）＋單一寫入點紀律（條目檔 frontmatter 是唯一寫入點、`MEMORY.md` 索引是其機械投影禁手寫；desc 禁 commit hash／流水；弧結案時同步蒸餾條目為終態 facts——結案兩步第三動）＋**body 形態：寫入當下即蒸後形**（user 09-06 拍板「不要寫一堆廢話後來再 audit」——lesson-first、禁 timeline/in-flight 細節；新建條目 >3,000 chars hook 硬擋）——完整規範見 memory-audit skill「寫入端紀律」段（觸發詞：一句話測試、寫入六問、任務終態、cluster-first、索引投影、desc 上限、結案蒸餾、body 形態、確定才寫、歸因未定）。
