...
3. **深審派發**：external family、[work-order review variant](../_common/work-order.md)。family 解析＝[model-routing 跨家族解析表](../model-routing/SKILL.md)（**與 caller 相異**是派發理由本身）：未指定 → GLM/ZCode 與 codex caller→muse；**muse caller→fail-loud**（無合法相異家族可自動選——**停下要求 user 選擇**：顯式 `--family codex`，或明示接受同家族 degraded review〔caller-harness full dual-context 承接＋記錄〕，禁解析層自選降級）；顯式指定與 caller 同 family → fail-loud。派發內容＝凍結資料＋scope manifest＋輸出格式要求（見下）。**派發形態現況**：muse 腿經 bridge 自動化已實戰；codex 腿本弧為 user-relay——首次自動派發未驗證〔first-real-usage-pending〕，且失敗先例正是薄 prompt 繞過 work-order（context 不自足）——自動派發必須逐節填滿 review variant 工單，首跑實測後回報
