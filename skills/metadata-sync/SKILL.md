...
| **EP 歸檔** | A, D | **歸檔前查證（防 ghost-done）**：列 EP 交付物（UC盤點/收尾/各段 deliverable）逐項 rg/fd/Read 驗落地——「段落完成」≠ codebase 真有（曾發生整份 EP 100% ghost-done 誤歸檔）；有 ghost-done 不歸檔（補做或標 🔧）。全綠才歸檔 → **task 目錄整搬** `mv 00-tasks/<task>/ 00-tasks/_done/<YYYY>/`（按年分層；EP/spec/Report Shell 同目錄一起走——目錄級整搬非單檔 mv；`_done/<YYYY>/` 不存在先建；舊 `ai-analysis/execution-plans/` 慣例退役）;綱要 EP(blueprint)等所有衍生子 EP 完成才歸檔 master |
...
| 漏項 | 觸發訊號(變更檔集模式) | 窄驗證 |
|------|----------------------|--------|
| EP 歸檔漏 | EP 段落交付物已 commit(code/test)且 `00-tasks/` 對應 task 目錄仍未歸檔（未搬 `_done/`） | `fd -e md -E _done . 00-tasks/`(單 call,涵蓋 task 目錄內 ep.md/spec.md,排除已歸檔)比對已交付未歸檔 |
