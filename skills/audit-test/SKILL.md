...
**與靜態隱含覆蓋的區別（/human-review 判準 2 補充）**：本角度（動態過時）與「靜態他處已測 = 冗餘」語義不同 — 後者指測試的行為已被**另一個測試**隱含驅動（如測 thin wrapper，但 wrapper 本體已被別處測；或同行為不同入口重複測）。靜態隱含覆蓋的查證與判斷見 [human-review skill](../human-review/SKILL.md) 判準 2；本命令聚焦動態過時 + 反模式，不重疊。
