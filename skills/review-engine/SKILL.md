...
6. **dual-context 編排（code-review Agent Tool 模式用）**：點 4 的 ①clean + ②UC-anchored 錨定二分法在 code-review 場景的 2-agent 變體 —— `code-reviewer`（fresh-eyes，只餵 diff）+ `code-reviewer-primed`（餵 diff + EP + Capabilities + dependency-graph（＋消費命令的條件式附加項，如 code-review 模式 B 的 transition 報告——定義在各消費命令）；意圖對齊 / 架構契合 / 測試精簡度 / 完整度光譜）。**context 差異在 spawn prompt 餵料，非 agent 定義**（subagent 自動注入 AGENTS.md，「空 context」靠不餵意圖文件達成）。中型以上（≥3 files）才 dual；小型單 fresh-eyes。**findings 合併規則**：同 file:line 去重；**矛盾不裁決** —— 合併端自行裁決 = 球員兼裁判，標 `conflict` 欄（兩方觀點並列）交 judge-review 裁決層。多樣性 > 數量：絕不 spawn 第二個同 context 的 fresh-eyes（共享盲點，邊際覆蓋 ≈0，同點 5 原則）。
