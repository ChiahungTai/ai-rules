...
確認後 `git add`（**納入本次開發的完整產物**：主變更 + build 階段 5a 結算的 finalization 檔 —— instruction 檔（AGENTS.md 為主，legacy CLAUDE.md）/ `.kanban/` / `00-tasks/`（EP＋spec＋Report Shell，含 `_done/` 歸檔搬移）/ `.tours/delta/`（持久 delta tour——post-build hook 2 產）/ flow-feedback 歸檔）+ `git commit`（含 attribution footer：`Co-Authored-By: <當前 harness 名>`——ZCode session 寫 `Co-Authored-By: ZCode`）。
