...
> **code-reality（若在場）**：Agent Review 的 wiring 驗證（新/改 symbol 的消費端接對沒）用 callers 查詢；段 impact 驗證用 `impact_radius`；「沒影響 X」的 claim 機械反證用 code-reality（graph 是 L1 機械證據，補強 Claim→Evidence→Trust，見階段 3 整合路徑檢查）。**CR 查詢分層**：主 session **MCP 優先**（MCP `callers`／`impact_radius`；資料面 `build`／`snapshot`／`delta_tour`／`project` MCP face 同在，v0.6.0 起）；build 的 review agents＝`Explore`（無 CR MCP 白名單）→ CLI 形態寫進 spawn prompt（callers＝`code-reality scip_refs <sym> --callers --repo <repo>`；impact＝`code-reality graph_query impact_radius --repo <repo> --files <絕對路徑>`）；跨 session `/code-review` 的 registry code-reviewer 族已掛白名單、定義檔自帶 MCP-first。分工 + GATE 見 [cr-query](../cr-query/SKILL.md)。
