...
> **code-reality（若在場）**：Agent Review 的 wiring 驗證（新/改 symbol 的消費端接對沒）用 callers 查詢；段 impact 驗證用 `impact_radius`；「沒影響 X」的 claim 機械反證用 code-reality（graph 是 L1 機械證據，補強 Claim→Evidence→Trust，見階段 3 整合路徑檢查）。**spawned review agent 未掛 code-reality MCP 工具——CR 查詢一律 CLI 形態**（callers＝`code-reality scip_refs <sym> --callers --repo <repo>`；impact＝`code-reality graph_query impact_radius --repo <repo> --files <絕對路徑>`）。分工 + GATE 見 [cr-query](../cr-query/SKILL.md)。
