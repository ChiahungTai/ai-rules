...
description: 當用戶獲得其他 AI 建議時進行批判性思考分析。基於深層思考框架（第一性原理 + 第二層思考）評估設計建議，並查證當前 codebase 的真實實作情況。適用於評估外部 AI 建議、架構設計決策、技術選型等場景。
...
**批判性分析 + 實作查證** — 基於深層思考框架（第一性原理 + 第二層思考）分析其他 AI 的建議，查證 codebase 真實實作，提供客觀的技術決策支援。

深層思考框架定義見 `~/.claude/rules/deep-thinking.md`（自動載入）。

### 核心職責
- **第一性原理分析**：從基本原理出發，拆解建議的本質
- **第二層後果追蹤**：追蹤建議的連鎖後果（強制）
...
思考框架定義見 `~/.claude/rules/deep-thinking.md`（自動載入）。以下為本 skill 的專用分析維度。
...
第一性原理分析完成後，必須追蹤至少兩層後果：

- **第一層後果**：採納建議的直接影響是什麼？拒絕建議的直接影響是什麼？
- **第二層後果**：第一層影響會導致什麼連鎖反應？是否觸發其他模組的連帶修改？
- **如果判斷錯誤**：採納後發現不對，回退成本是多少？拒絕後發現應該採納，補救成本是多少？
