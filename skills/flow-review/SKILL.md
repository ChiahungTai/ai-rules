...
> **歸檔生命週期**：決策且對應變更經 `/implement`→`/commit` 落地 → **build 階段 5a 結算（情境 A）** 把該 feedback 歸檔到 `ai-analysis/flow-feedback/_done/`（同 EP `_done/` 哲學，見 [metadata-sync](../metadata-sync/SKILL.md) flow-feedback 歸檔項）；「還沒想清楚」留 root 供下次 review。active inbox（root）= 未結案項。
