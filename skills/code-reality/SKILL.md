...
| `tour register`／`tour materialize <arcId>` | **intent-level delta materialization 兩階段**（AIR-80）：register＝持久化 `[[delta_arc]]` pending row（ask-once 略過路徑——缺 row＝消費端永久不可達）；materialize＝row-driven 組完整 recipe（commit-ish 經 rev-parse 解析→snapshot pair sha8 查找 fail-loud→**stale gate**〔`_meta.stale` 非空即拒〕→EP repo-relative 解析〔絕對路徑不進 tour step〕）→ `.tours/delta/<arcId>.tour`＋row 補 tourPath/quality；**同 arcId 重產＝覆蓋同 tourPath**（canonical key、歷史交 git）；delta row＝tool-owned full replace——契約細節見 [tour-bootstrap](../tour-bootstrap/SKILL.md)「manifest provenance 契約」 |
