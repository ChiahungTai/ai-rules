...
| `tour materialize <arcId>` | **intent-level delta materialization**（AIR-80）：依 manifest `[[delta_arc]]` provenance row 組完整 recipe（snapshot pair sha8 解析 fail-loud＋EP gate＋build）→ `.tours/delta/<arcId>.tour`＋row upsert（tourPath/quality）；首次帶 `--base`＋`--target`（`--ep`/`--card` 選配）註冊；**同 arcId 重產＝覆蓋同 tourPath**（canonical key、歷史交 git）；row 保留不刪；arcId＝materialization canonical key、cardId＝join 屬性（一卡多弧）——契約細節見 [tour-bootstrap](../tour-bootstrap/SKILL.md)「manifest provenance 契約」 |
