...
**應該新增**（High Signal）：設計理由、架構約束、非顯而易見的選擇、模組邊界、失敗教訓、分類判準與座標行（AIR-45「Capabilities desc 文法與分類層級」節，定義源見 instruction-writing skill）、S4 處置後的寫回澄清（觸發狀態／降級標記，禁只改散文不改判準行）
...
> **同步哲學**: instruction 檔是活文檔，必須與程式碼同步演進。當 sync 發現 ⚠️ ≥ 3 個時，建議執行 `/instruction-sync {module} --all` 完整 12 角度深度驗證。
