...
**不做的事**：逐行正確性 → `/code-review`；結構 / 重用 → `/illustrate`（結構 viewport）；跑測試 / 覆蓋率 → `/implement` + `/audit-test`。
...
按需讀取：
- [review-engine](../review-engine/SKILL.md) — LSP 查證方法（Phase 2）+ 通用審查邏輯（嚴重度/自證/多層驗證）
...
判定規則（effort/max-agents → 模式）見 [review-engine](../review-engine/SKILL.md)；max-agents 上限查 [agent-workflow 並發表](../agent-workflow/SKILL.md)。下表為交付渲染的平行化判定（借用相同條件）。**注意：review-engine 的 force 獨立（取消 Main LLM 自審）僅適用 review 命令（Writer/Reviewer 分離需求）；交付渲染無此需求，Main LLM 序列渲染合法**（Phase 2 互動必須 Main LLM）：
...
──────────────────────────────────────────────────────────────────────────────────────────
deliverable-review(docs/rules)  審查渲染 demo-checklist(非 selective tier)  post-build 驗收流程
illustrate 結構 viewport       審查渲染結構拓樸 + 重用枚舉                 post-EP/implement 結構審查
...
/spec（純輔助·需求釐清，可選）→ /execution-plan（含 EP Review）
        ...
        /deliverable-review --ep（layer 3，planned）→ /illustrate --ep（layer 3，結構 viewport）
          ↓
        /implement（含 Agent Review + /audit-test）
