...
- `code-reality` — code_reality 工具鏈程序層（meta 層工具，Rust carrier `code-reality <tool> --repo`、住獨立 repo `~/Github/code-reality`：snapshot／transition／hub_refs（含 hazard 分層安全網——防「0 refs 可刪」誤判）／runtime_edges／boundary／boundary_build／delta_tour／chain_tour／graph_csv／graph_audit（Rust 完整度稽核）／scip_refs（SCIP 索引 refs 真相源——CRG 同鍵去重受害符號）／tour_validate／tour_upgrade／tour_manifest；repo profile `.code-reality.toml` schema、存在性偵測單一真相源、claims 口徑限制；與 crg-query 分工：CRG 查圖譜結構、code_reality 做邊集導出與 EP 對照）
