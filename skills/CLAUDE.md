...
- `/compact-prep` — /compact 前置外部化：落檔 raw tail＋memory 新鮮度檢查，user 再手動 /compact；tail 注入已由 SessionStart(compact) hook（`hooks/compact-tail-inject.py`）自動化的環境，職責縮減為 memory 檢查
