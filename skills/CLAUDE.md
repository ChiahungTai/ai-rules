...
- `code-reality` — code_reality 工具鏈程序層（meta 層工具住 ai-rules：snapshot／transition／hub_refs／runtime_edges／boundary／boundary_build／delta_tour／chain_tour／graph_csv／graph_audit（Rust 完整度稽核）／tour_validate／tour_upgrade／tour_manifest；repo profile `.code-reality.toml` schema、存在性偵測單一真相源、claims 口徑限制；與 crg-query 分工：CRG 查圖譜結構、code_reality 做邊集導出與 EP 對照）
