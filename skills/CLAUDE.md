...
- `corrections-weekly` — 糾正模式週報＋CR 使用健檢＋memory 寫入歸因（排程用，週六 23:10 ZCode cron 開新 session）：腳本撈 ZCode db 糾正候選＋LLM 判讀分類，append 月檔 `ai-analysis/reports/corrections-<YYYY-MM>.md`；某類暴增＝規則衰減訊號；memory 段＝AIR-40 telemetry（top actor×entry 寫入排行——subagent 大戶抽驗線索）
...
- `model-routing` — subagent 模型分層深層載體（reference skill：tier→(model,effort) 解析表〔中文標籤旗艦/一般〕＋旗艦資格條款（五項）／坐位註記＋lite 分工律〔執行層條件式降級＝保護面厚度、判斷密集位 full 能力檔、模型歸因紀律〕＋external-runtime family→(model,effort,容量) 解析表＋eligibility gate／reviewer 交接契約／完成回報收法（fire-and-forget 決策樹：--background 提交＋跨 session 認領＋wait/show 晚收、timeout 訊號家系拆分、ETA-gate fallback）／套用三路徑、rate limit 並發表、thoughtLevel 但書；rule 端留角色→tier 表＋詞彙定義＋兩跳骨架——rule+skill 分層控制 bundle 尺寸）
