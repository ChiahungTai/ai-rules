...
- `tour-bootstrap` — repo 導覽建置程序（三層 corpus：Overview 地圖／Chain 場景／Delta 時間；`.tour` 語言契約——CodeTour 消費端正則決定的 line/pattern/tour link/file link 規則；機械驗證清單＋AI 不代終審停點；建在 code-reality 工具層之上，斷點③＝AI 輔助 callchain 生成待沉澱）
