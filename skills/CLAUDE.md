...
- `crg-query` — code-review-graph 知識圖譜查詢紀律（LSP-vs-CRG-vs-code-reality 三員分工：symbol→LSP（Rust repo 符號/caller 面 SCIP 在場可走 code-reality）／impact·callers·flows·community→CRG；assume-present + warn-if-absent；anti-over-reliance：graph=structure 非 behavior；CRG 裝了才 fire，平行 nt-query）
