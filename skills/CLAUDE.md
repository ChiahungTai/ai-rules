...
- `/mosaic-idle-tasks` — mosaic_alpha 閒時任務範本產生器（ZCode 免費 idle 通道、UI copy-paste 建立；四張：weekly-standup / test-findings / doc-sync / research-scan；範本自足——唯讀、禁背景子智能體、分段續跑鐵律逐張內嵌；例行工作走 CronCreate 定時非閒時）
...
- `crg-query` — code-review-graph 知識圖譜查詢紀律（LSP-vs-graph-vs-code-reality 分工：symbol 真相→code-reality SCIP（Rust）／LSP（hover/簽名/即時）／impact·callers·flows·community→code-reality engine（graph_query 家族，讀 .code-reality/graph.db 自有格式；CRG MCP 已於 2026-08-26 cutover 退役）；assume-present + warn-if-absent；anti-over-reliance：graph=structure 非 behavior；CRG 裝了才 fire，平行 nt-query）
- `code-reality` — code_reality 工具鏈程序層（meta 層工具，Rust carrier `code-reality <tool> --repo`、住獨立 repo `~/Github/code-reality`：snapshot／transition／hub_refs（含 hazard 分層安全網——防「0 refs 可刪」誤判）／runtime_edges／boundary／boundary_build／delta_tour／chain_tour／graph_audit（Rust 完整度稽核）／scip_refs（SCIP 索引 refs 真相源——CRG 同鍵去重受害符號）／tour_validate／tour_upgrade／tour_manifest——全工具讀 `.code-reality/graph.db` 自有格式（2026-08-27 cutover，`.code-review-graph/` 僅剩 import_legacy 匯入源）；repo profile `.code-reality.toml` schema、存在性偵測單一真相源、claims 口徑限制；與 crg-query 分工：CRG 查圖譜結構、code_reality 做邊集導出與 EP 對照）
