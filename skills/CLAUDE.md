...
- `crg-query` — code-review-graph 知識圖譜查詢紀律（LSP-vs-graph-vs-code-reality 分工：symbol 真相→code-reality SCIP（Rust）／LSP（hover/簽名/即時）／impact·callers·flows·community→code-reality engine（graph_query 家族；CRG MCP 已於 2026-08-26 cutover 退役）；assume-present + warn-if-absent；anti-over-reliance：graph=structure 非 behavior；CRG 裝了才 fire，平行 nt-query）
