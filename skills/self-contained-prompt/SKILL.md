...
把工作交給另一個 LLM / session / repo 時，**對方讀不到你的對話 context**（決策脈絡、進度、為何這樣做都只在對話裡）。本 skill 是把這些「對話結晶」打包成 self-contained prompt 的原則唯一真相源，被 [`/handoff`](../../commands/handoff.md)（產交接 prompt）與 [agent-review-cycle](../../commands/claude/_common/agent-review-cycle.md)（subagent prompt）共用。
...
| [agent-review-cycle](../../commands/claude/_common/agent-review-cycle.md) | subagent prompt 也遵循本原則（同環境・審查型）| 3-perspective 執行範本 |
