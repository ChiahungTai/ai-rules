...
>
> **核心理念**：**人類產出格式優先**（genesis 教訓——前次設計失敗都在重造 AI 機械、欠投資人類產出 → token 牆 → 「人類不會看」）。本 mode 把設計預算花在**人類可讀產出**（廣到精細 + 受眾分離 + emoji-first Mermaid），AI 機械（tiering/CRG/渲染）委外給既有 skills。
>
...
| `/smell-detector --baseline`（無 dir） | 印 status + 用 CRG 目錄級 importer 聚合挑下一個 pending core 目錄（或問） |
...
| `/smell-detector --architecture` (`--arch`) | (重)產 codebase 級 `ai-analysis/codebase-review/architecture.md`（CRG onboarding 全圖） |
...
2. 若不同步 → 語料重生（Python：`python ~/Github/code-reality/scripts/lsp_harvest.py`；Rust：重生 SCIP index）→ `code-reality graph_db build --repo <root>`（單一 build 面、冪等）；舊 `.code-review-graph/` 在場 → 接 `graph_db import_legacy`。
...
- **預設直接執行**（單 session）：目錄核心檔通常 ~6 個，§zoom 深審分析 fit 一個 session（common/ + indicators/ 實證）。讀 reference → 讀核心檔 → 真實 `uv run`/LSP/CRG → 寫 4 檔。
...
> 廣到精細 + 受眾分離。委託：[arch-thinking](../arch-thinking/SKILL.md)（core/leaf tiering + City Map）+ [crg-query](../crg-query/SKILL.md)（CRG facts）+ [illustrate](../illustrate/SKILL.md)（渲染）+ [review-engine](../review-engine/SKILL.md)（severity/confidence only）。
