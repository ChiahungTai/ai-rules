...
```bash
# 根據 skill 建議的最優並行度執行（預設 4-6 個並行任務）
Task 1 (document-quality-checker): 處理 skill 建議的檔案組 1 "
...
Task 4 (critical-thinking-analyzer): 處理 skill 建議的檔案組 1 "
  - 第一性原理檢查：基本假設驗證、邏輯鏈條分析
  - 批判性思考：替代方案考慮、本質問題識別
  - 思維深度：是否基於事實而非經驗推測
" &

Task 5 (critical-thinking-analyzer): 處理 skill 建議的檔案組 2 "
  - 第一性原理檢查：方案選擇理性評估、長期可持續性
  - 批判性思考：因果關係驗證、邏輯謬誤識別
  - 思維深度：是否處理表面症狀或本質問題
" &
...
# 生成綜合報告
Task report-coordinator "整合所有分析結果，包含第一性原理評估，生成詳細品質評估報告和優先級改進建議"
```

#### **深度模式 (--deep) - 包含外部驗證**
深度分析 + 外部驗證 + 第一性原理批判性思考：

```bash
# 根據 skill 建議的並行度執行深度檢查（預設 6-8 個並行任務）
Task 1 (document-quality-checker): 處理 skill 建議的檔案組 1 "
...
Task 4 (external-validator): 並行執行外部驗證任務 "
  ...
  - 程式碼倉庫路徑驗證
" &
Task 5 (critical-thinking-analyzer): 處理 skill 建議的檔案組 1 "
  - 深度第一性原理檢查：基本假設事實驗證、邏輯鏈條完整性驗證
  - 高級批判性思考：多維度替代方案分析、本質問題深度識別
  - 外部驗證整合：結合外部資料進行批判性分析
" &
Task 6 (critical-thinking-analyzer): 處理 skill 建議的檔案組 2 "
  - 深度第一性原理檢查：方案選擇的理性重構、長期可持續性驗證
  - 高級批判性思考：因果關係深度驗證、系統性邏輯謬誤檢測
  - 外部驗證整合：對比外部最佳實踐進行批判性評估
" &

wait

# 高級分析和建議
Task report-coordinator "整合深度分析結果，生成包含外部驗證和第一性原理批判性思考的完整品質報告和實施路線圖"
```
...
- **🟡 重要問題（建議修復）**
  ...
  - [ ] 內容重複或冗餘
  - [ ] **第一性原理問題：未考慮合理的替代方案**
  - [ ] **第一性原理問題：處理表面症狀而非本質問題**
  - [ ] **第一性原理問題：方案選擇缺乏充分的理性依據**
...
- **🟢 建議性問題（可選優化）**
  ...
  - [ ] 可以增加更多實用範例
  - [ ] 文檔格式可以統一
  - [ ] **第一性原理建議：可以更深入地質疑基本假設**
  - [ ] **第一性原理建議：可以增加更多維度的思考角度**
...
### 文檔品質分析報告（包含第一性原理批判性思考）

在 target_path 中發現 5 個品質問題：

1. **第一性原理缺陷：基本假設錯誤** - API 設計假設缺乏驗證 (信心分數: 95)
   - 位置：docs/api-design.md:15-22
   - 問題：API 設計基於「用戶會按照預期方式使用」的未驗證假設
   - 第一性原理分析：未考慮用戶實際使用場景的複雜性
   - 建議：基於用戶研究和數據驗證重新設計 API

2. **邏輯矛盾** - API 版本描述不一致 (信心分數: 95)
   - 位置：docs/api.md:45 vs docs/api.md:123
   - 問題：同一 API 在不同章節描述了不同的版本號
   - 第一性原理分析：版本管理缺乏統一的理性標準
   - 建議：統一版本資訊，確認正確的 API 版本

3. **無效程式碼路徑** - 引用的檔案不存在 (信心分數: 100)
   - 位置：docs/examples.md:67
   - 問題：引用的程式碼路徑 `examples/advanced/demo.py` 不存在
   - 第一性原理分析：缺乏自動化檢查機制，依賴人工維護
   - 建議：修正路徑或移除無效引用，建立自動化驗證

4. **第一性原理問題：未考慮替代方案** - 架構選擇缺乏比較 (信心分數: 85)
   - 位置：docs/architecture.md:34-50
   - 問題：只描述了當前架構，未分析其他可能的架構方案
   - 第一性原理分析：選擇當前方案的理性依據不充分
   - 建議：補充架構方案比較分析，提供選擇依據

5. **術語不一致** - 同一概念使用不同術語 (信心分數: 80)
   - 位置：多處
...
#### 第一性原理子指標:
- **基本假設驗證**: 60/100 (需改善) - 存在未經驗證的假設
- **邏輯鏈條完整性**: 75/100 (中等) - 大部分邏輯完整但存在跳躍
- **替代方案包容性**: 65/100 (需改善) - 未充分考慮替代方案
- **本質問題對應**: 70/100 (中等) - 基本解決問題但深度不足
...
## 🚀 改善建議
1. **立即修復**: 統一 API 版本描述，修正無效路徑，驗證基本假設
2. **近期改善**: 建立統一術語表，補充架構方案比較，優化章節順序
3. **長期維護**: 定期檢查外部連結，建立內容審核流程，建立第一性原理檢查清單

## 🧠 第一性原理改善建議
- **質疑機制**: 在技術決策文檔中加入「基本假設驗證」章節
- **替代分析**: 對重要選擇提供多方案比較和選擇依據
- **本質思考**: 強化從根本問題出發的解決方案設計
- **批判檢視**: 建立內部批判性思考審核流程
...
在 target_path 中未發現顯著品質問題，第一性原理思維品質優秀。
...
#### 第一性原理子指標:
- **基本假設驗證**: 92/100 (優秀) - 所有關鍵假設都有充分驗證
- **邏輯鏈條完整性**: 88/100 (優秀) - 邏輯推理完整嚴謹
- **替代方案包容性**: 90/100 (優秀) - 充分考慮並分析了替代方案
- **本質問題對應**: 85/100 (優秀) - 準確識別並解決了本質問題
...
## 🎉 卓越表現
- **第一性原理應用**: 文檔展現了優秀的批判性思考，從基本假設驗證到最終結論的邏輯鏈條完整嚴謹
- **全面性分析**: 不僅解決了表面問題，更深入分析了本質原因和多重可能性
- **理性決策**: 重要的技術選擇都有充分的替代方案比較和理性的選擇依據
...
- **內容清晰度 (20%)**: 語言表達是否清晰易懂
- **結構邏輯性 (15%)**: 章節安排是否合理
- **一致性 (20%)**: 內容是否自洽，術語是否統一
- **連結準確性 (10%)**: 連結和引用是否有效
- **完整性 (10%)**: 重要內容是否完整
- **第一性原理思維 (25%)**: 基於本質的批判性思考品質

#### 第一性原理評分子指標
- **基本假設驗證 (30%)**: 文檔假設是否基於事實和邏輯
- **邏輯鏈條完整性 (25%)**: 從前提結論的邏輯鏈條是否完整
- **替代方案包容性 (25%)**: 是否考慮多種可能性和選擇理性
- **本質問題對應 (20%)**: 是否真正解決了本質問題而非表面症狀
...
- [ ] 已排除 false positive
- [ ] 已按優先級分類問題（包含第一性原理問題）
- [ ] 已提供具體改善建議（包含第一性原理改善建議）
- [ ] 已計算品質評分（包含第一性原理子指標）
- [ ] 已使用正確的輸出格式（包含第一性原理分析）
- [ ] 已檢查通過條件（包含第一性原理檢查項）
- [ ] 已提供量化的品質指標（包含第一性原理指標）
